...
function getServiceHeaders(serviceKey: string) {
  return {
    apikey: serviceKey,
...
async function isAdmin(userId: string, supabaseUrl: string, serviceKey: string): Promise<boolean> {
  const headers = getServiceHeaders(serviceKey);
...
Deno.serve(async (req: Request) => {
  ...
  // Strip the edge function prefix from the path
  let path = url.pathname.replace(/^\/+|\/+$/g, "");
  path = path.replace(/^functions\/v1\/users-api\/?/, "");
  ...
  try {
    ...
    if (action === "teams") {
      const headers = getServiceHeaders(serviceKey);
      const result = await fetch(`${supabaseUrl}/rest/v1/teams?select=id,name,slug,description&order=name.asc`, { headers });
    ...
    if (req.method === "POST" && action === "teams") {
      ...
      const admin = await isAdmin(userId, supabaseUrl, serviceKey);
    ...
    if (req.method === "GET" && !action) {
      ...
      // Brief mode returns only id, name, email for dropdowns
      const selectFields = brief ? "id,name,email" : "*,teams(id,name,slug)";

      let query = `${supabaseUrl}/rest/v1/profiles?select=${selectFields}&order=name.asc&limit=${limit}&offset=${offset}`;
