  ...
  try {
      ...
      return jsonResponse({
        ...
        endpoints: [
          ...
          { method: "GET", path: "/teams", description: "List all teams" },
          {
            method: "POST",
            path: "/teams",
            description: "Create team (admin only)",
          },
    ...
    if (req.method === "GET" && action === "teams") {
      ...
      const result = await fetch(
        `${supabaseUrl}/rest/v1/teams?select=*&order=name.asc`,
        { headers },
      );
      const teams = await result.json();
      if (!result.ok)
        return errorResponse(
          teams.message || "Failed to list teams",
          result.status,
        );
    ...
    if (action === "me") {
      ...
      const headers = getAuthHeaders(req, serviceKey);
      const result = await fetch(
        `${supabaseUrl}/rest/v1/profiles?id=eq.${userId}&select=*,teams(id,name,slug)`,
        { headers },
      );
    ...
    if (req.method === "GET" && action) {
      const headers = getAuthHeaders(req, serviceKey);
      const result = await fetch(
        `${supabaseUrl}/rest/v1/profiles?id=eq.${action}&select=*,teams(id,name,slug)`,
        { headers },
      );
      const users = await result.json();
      if (!users || users.length === 0) {
    ...
    if (req.method === "PUT" && action) {
      ...
      const result = await fetch(
        `${supabaseUrl}/rest/v1/profiles?id=eq.${action}`,
        {
          method: "PATCH",
          headers: { ...headers, Prefer: "return=representation" },
